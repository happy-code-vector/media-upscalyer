...
def upscale_image(input_path, output_path, upsampler, target_resolution):
    ...
    try:
        ...
        scale_needed = target_width / current_width
        
        current_img = img
        passes_needed = 0
        
        # Apply 4x upscaling until we reach or exceed target resolution
        # while (current_img.shape[1] < target_width or current_img.shape[0] < target_height) and passes_needed < 3:
        #     print(f"Applying upscaling pass {passes_needed + 1}...")
        current_img, _ = upsampler.enhance(current_img)
        passes_needed += 1
        print(f"Current resolution after pass {passes_needed}: {current_img.shape[1]}x{current_img.shape[0]}")
        
        # Final resize to exact target resolution
        final_img = cv2.resize(current_img, target_resolution, interpolation=cv2.INTER_LANCZOS4)
        cv2.imwrite(output_path, final_img)
        end_time = time.time()
        print(f"Successfully upscaled image to {output_path} using {passes_needed} pass(es)")
...
def upscale_video(input_path, output_path, upsampler, target_resolution):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        for i, frame_name in enumerate(frame_files):
            ...
            try:
                # Apply multiple passes if needed for high scaling factors
                current_img = img
                target_width, target_height = target_resolution
                passes_needed = 0
...
def main():
    ...
    if args.scale:
        if args.scale <= 2:
            ...
            model_scale = 4
        elif args.scale <= 16:  # Support up to 16x by applying 4x multiple times
            model_scale = 4
        else:
            print("Error: Scaling factors greater than 16 are not yet supported.")
            exit()
        ...
        if args.target_resolution in RESOLUTION_MAP:
            ...
            if scale_factor <= 2:
                ...
                model_scale = 4
            elif scale_factor <= 16:  # Support up to 16x by applying 4x multiple times
                model_scale = 4
            else:
                print("Error: Scaling factors greater than 16 are not yet supported.")
                exit()
